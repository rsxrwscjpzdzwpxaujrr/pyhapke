...
class HapkeRTM:
    def __init__(self, i = 0, e = np.pi/6, g = np.pi/6, P = P_WAC, wl = None, poros = 0.41, Bs0 = 1, Bc0 = 0, hs = None):
        ...
        self.mu = np.cos(e)
        self.hs = hs
        self.Bs = compute_Bs(poros = poros, g = g, hs = hs) #"Classic" SHOE version of backscatter
        self.Bs0 = Bs0
        self.Bc0 = Bc0
    ...
    def hapke_function_RADF(self, ssa):
        ...
        R = (ssa/4) * mu0/ (mu0 + mu) * ((1 + Bs * Bs0) * P + H * H0 - 1) * (1 + Bc * Bc0)
    ...
    def hapke_function(self, ssa):
        ...
        #R = (ssa/4) * mu0 / (mu0 + mu) * ((1 + B) * P + H * H0 - 1)
        R = (ssa/4 / np.pi)  * mu0 / (mu0 + mu) * ((1 + Bs * Bs0) * P + H * H0 - 1) * (1 + Bc * Bc0)
